...
class MonarchAttnImplicitFn(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, V, sm_scale, num_iters, eps):
        ...
        L = torch.eye(block_b1, device=Q.device, dtype=Q.dtype).view(1, 1, 1, 1, block_b1, block_b1).expand(b, h, frames, block_b2, block_b1, block_b1) # (b, h, f, j, k, i)

        with torch.no_grad():
            for _ in range(num_iters):
                aR = torch.einsum("bhfjki,bijhd->bfkjhd", L, Q)
                bR = torch.einsum("bfkjhd,bfklhd->bhfkjl", aR, K)
                # cR = torch.einsum("bhjki->bhkj", L_star).unsqueeze(-1)
                # R = torch.softmax(bR / (cR + eps), dim=-1)
                cR = L.sum(dim=-1, dtype=torch.float32, keepdim=True).clamp_min(eps).transpose(-2, -3) # (b, h, f, k, j, 1)
                z = bR.to(torch.float32) * (1.0 / (cR + eps)).clamp_max(1e4)
                z = z - z.amax(dim=-1, keepdim=True)
                R = torch.softmax(z, dim=-1).to(Q.dtype)

                aL = torch.einsum("bhfkjl,bfklhd->bfjkhd", R, K)
                bL = torch.einsum("bfjkhd,bijhd->bhfjki", aL, Q)
                # cL = torch.einsum("bhkjl->bhjk", torch.xlogy(R, R)).unsqueeze(-1)
                logz = torch.logsumexp(z, dim=-1, keepdim=True) # (b, h, f, k, j, 1)
                cL = (R * (z - logz)).sum(dim=-1, keepdim=True).transpose(-2, -3) # (b, h, f, j, k, 1)
        ...
        out = torch.einsum("bhfkjl,bfklhd->bfjkhd", R, V)
    ...
    def backward(ctx, grad_out):
        ...
        grad_tmp = torch.einsum("bijhd,bhfjki->bfjkhd", grad_out, L_star)
        grad_V = torch.einsum("bhfkjl,bfjkhd->bfklhd", R_star, grad_tmp)
        grad_R = torch.einsum("bfjkhd,bfklhd->bhfkjl", grad_tmp, V)

        def R_from_QK(Q_in, K_in):
            aR = torch.einsum("bhfjki,bijhd->bfkjhd", L_star, Q_in)
            bR = torch.einsum("bfkjhd,bfklhd->bhfkjl", aR, K_in)
            # cR = torch.einsum("bhjki->bhkj", L_star).unsqueeze(-1)
            # R = torch.softmax(bR / (cR + eps), dim=-1)
            cR = L_star.sum(dim=-1, dtype=torch.float32, keepdim=True).clamp_min(eps).transpose(-2, -3) # (b, h, f, k, j, 1)
            z = bR.to(torch.float32) * (1.0 / (cR + eps)).clamp_max(1e4)
            z = z - z.amax(dim=-1, keepdim=True)
        ...
        def O_from_L(L_in):
            aR = torch.einsum("bhfjki,bijhd->bfkjhd", L_in, Q)
            bR = torch.einsum("bfkjhd,bfklhd->bhfkjl", aR, K)
            # cR = torch.einsum("bhjki->bhkj", L_in).unsqueeze(-1)
            # R = torch.softmax(bR / (cR + eps), dim=-1)
            cR = L_in.sum(dim=-1, dtype=torch.float32, keepdim=True).clamp_min(eps).transpose(-2, -3) # (b, h, f, k, j, 1)
            z = bR.to(torch.float32) * (1.0 / (cR + eps)).clamp_max(1e4)
            z = z - z.amax(dim=-1, keepdim=True)
            R = torch.softmax(z, dim=-1).to(L_in.dtype)

            out = torch.einsum("bhfkjl,bfklhd->bfjkhd", R, V)
            out = torch.einsum("bhfjki,bfjkhd->bijhd", L_in, out)
            return out
        ...
        def L_from_L(L_in):
            ...
            aL = torch.einsum("bhfkjl,bfklhd->bfjkhd", R_out, K)
            bL = torch.einsum("bfjkhd,bijhd->bhfjki", aL, Q)
            # cL = torch.einsum("bhkjl->bhjk", torch.xlogy(R_out, R_out)).unsqueeze(-1)            
            logz = torch.logsumexp(z, dim=-1, keepdim=True) # (b, h, f, k, j, 1)
            cL = (R_out * (z - logz)).sum(dim=-1, keepdim=True).transpose(-2, -3) # (b, h, f, j, k, 1)
            L_out = rearrange(bL - cL, 'b h f j k i -> b h j i (f k)')
            L_out = torch.softmax(L_out, dim=-1).to(L_in.dtype)
            L_out = rearrange(L_out, 'b h j i (f k) -> b h f j k i', f=frames, k=block_b1)
            return L_out
        ...
        def L_from_QK(Q, K):
            aR = torch.einsum("bhfjki,bijhd->bfkjhd", L_star, Q)
            bR = torch.einsum("bfkjhd,bfklhd->bhfkjl", aR, K)
            # cR = torch.einsum("bhjki->bhkj", L_star).unsqueeze(-1)
            # R_out = torch.softmax(bR / (cR + eps), dim=-1)
            cR = L_star.sum(dim=-1, dtype=torch.float32, keepdim=True).clamp_min(eps).transpose(-2, -3) # (b, h, f, k, j, 1)
...
class MonarchAttention(nn.Module):
    ...
    def _forward_inference(self, x: torch.Tensor, image_rotary_emb, attention_mask, layer_kv_cache, eps=1e-6):
        ...
        q = self.norm_q(q).to(q.dtype)
        k = self.norm_k(k).to(k.dtype)
        ...
        q = proxy_apply_rotary_emb(q, image_rotary_emb)
        k = proxy_apply_rotary_emb(k, image_rotary_emb)
        ...
        for f in range(q.size(1)):
            ...
            out = monarch_attn(curr_q, curr_k, curr_v, self.sm_scale, self.num_iters, eps)
            outputs.append(out)
...
def FAR_B_TRUE_MONARCH_IMPLICIT_SLOW(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_M_TRUE_MONARCH_IMPLICIT_SLOW(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_L_TRUE_MONARCH_IMPLICIT_SLOW(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_XL_TRUE_MONARCH_IMPLICIT_SLOW(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_B_TRUE_MONARCH_IMPLICIT_FAST(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_M_TRUE_MONARCH_IMPLICIT_FAST(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_L_TRUE_MONARCH_IMPLICIT_FAST(**kwargs):
...
@MODEL_REGISTRY.register()
def FAR_XL_TRUE_MONARCH_IMPLICIT_FAST(**kwargs):
