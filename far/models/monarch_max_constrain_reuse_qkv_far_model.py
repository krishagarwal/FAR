...
class MonarchAttention(nn.Module):
    ...
    def _forward_inference(self, x: torch.Tensor, image_rotary_emb, attention_mask, layer_kv_cache):
        ...
        rope_q = proxy_apply_rotary_emb(q, image_rotary_emb)
        rope_k = proxy_apply_rotary_emb(k, image_rotary_emb)

        rope_q = rope_q.view(batch_size, -1, 8, 8, self.num_heads, self.head_dim)
        q = q.view(batch_size, -1, 8, 8, self.num_heads, self.head_dim)
        rope_k = rope_k.view(batch_size, -1, 8, 8, self.num_heads, self.head_dim)
        k = k.view(batch_size, -1, 8, 8, self.num_heads, self.head_dim)
        v = v.view(batch_size, -1, 8, 8, self.num_heads, self.head_dim)

        lk = self.local_k(rope_q[:, :, :, 0], rope_k, v, image_rotary_emb)
        if lk_cache is not None:
            lk = torch.cat([lk_cache, lk], dim=-4)
        L = torch.einsum('baijhd,bfkhd->bhafjik', rope_q, lk)

        rq = self.local_q(rope_q[:, :, 0], rope_k, v, image_rotary_emb)
        rk = rope_k
